...
def setup_image_node(material, image, active=True):
...
def bake_unity_textures(obj, export_path, albedo_res=1024, normal_res=1024, mrao_res=1024, 
    ...
    for node in material.node_tree.nodes:
        if node.type == 'TEX_IMAGE':
            original_nodes[node.name] = node
    
    print(f"\n📊 ДИАГНОСТИКА МАТЕРИАЛА:")
    nodes = material.node_tree.nodes
    principled = None
    for node in nodes:
    ...
    if principled:
        print(f"✓ Найден Principled BSDF: {principled.name}")
        print(f"  Metallic:")
        if principled.inputs['Metallic'].is_linked:
            metallic_link = principled.inputs['Metallic'].links[0]
            print(f"    - Подключено: {metallic_link.from_node.type} → {metallic_link.from_socket.name}")
        else:
            print(f"    - Константа: {principled.inputs['Metallic'].default_value}")
        
        print(f"  Roughness:")
        if principled.inputs['Roughness'].is_linked:
            roughness_link = principled.inputs['Roughness'].links[0]
            print(f"    - Подключено: {roughness_link.from_node.type} → {roughness_link.from_socket.name}")
        else:
            print(f"    - Константа: {principled.inputs['Roughness'].default_value}")
    else:
    ...
    try:
        ...
        if bake_albedo:
            ...
            setup_image_node(material, albedo_img, active=True)
        ...
        if bake_normal:
            ...
            setup_image_node(material, normal_img, active=True)
        ...
        if not bake_mrao:
            ...
            if principled:
                ...
                if principled.inputs['Metallic'].is_linked:
                    metallic_socket = principled.inputs['Metallic'].links[0].from_socket
                    
                    # Blender 5.0+ использует SeparateColor вместо SeparateRGB
                    try:
                        separate_rgb = nodes.new(type='ShaderNodeSeparateColor')
                        combine_rgb = nodes.new(type='ShaderNodeCombineColor')
                    except:
                        # Fallback для Blender 3.x-4.x
                        separate_rgb = nodes.new(type='ShaderNodeSeparateRGB')
                        combine_rgb = nodes.new(type='ShaderNodeCombineRGB')
                    
                    links.new(metallic_socket, separate_rgb.inputs[0])
                    links.new(separate_rgb.outputs[0], combine_rgb.inputs[0])
                    links.new(separate_rgb.outputs[0], combine_rgb.inputs[1])
                    links.new(separate_rgb.outputs[0], combine_rgb.inputs[2])
                    links.new(combine_rgb.outputs[0], emission.inputs['Color'])
                    
                    temp_nodes = [separate_rgb, combine_rgb]
                else:
                    value = principled.inputs['Metallic'].default_value
                    emission.inputs['Color'].default_value = (value, value, value, 1.0)
                    temp_nodes = []
                
                links.new(emission.outputs['Emission'], output.inputs['Surface'])
                
                setup_image_node(material, metallic_img, active=True)
                bake_map(obj, 'EMIT', metallic_img, samples)
            ...
            if principled:
                ...
                if principled.inputs['Roughness'].is_linked:
                    roughness_socket = principled.inputs['Roughness'].links[0].from_socket
                    ...
                    links.new(roughness_socket, separate_rgb.inputs[0])
                    links.new(separate_rgb.outputs[0], combine_rgb.inputs[0])
                    links.new(separate_rgb.outputs[0], combine_rgb.inputs[1])
                    links.new(separate_rgb.outputs[0], combine_rgb.inputs[2])
                    links.new(combine_rgb.outputs[0], emission.inputs['Color'])
                    ...
                    value = principled.inputs['Roughness'].default_value
                    emission.inputs['Color'].default_value = (value, value, value, 1.0)
                    temp_nodes = []
                
                links.new(emission.outputs['Emission'], output.inputs['Surface'])
                
                setup_image_node(material, roughness_img, active=True)
                bake_map(obj, 'EMIT', roughness_img, samples)
            ...
            setup_image_node(material, ao_img, active=True)
